...
- The client `loadUser` receives now sends the session access token, so a
  ...
  `settleAuthenticated` and `reloadUser` both hand that client to
  `loadUser`, the user load after a login or a refresh was unauthenticated. On
  ...
  A caller-supplied `client` is untouched and stays the caller's
  responsibility, which the option's documentation and the README now say.

  Every other package takes the lockstep version bump with no change.

- Updated dependencies
  - @webbpulse/api-client@0.10.6
